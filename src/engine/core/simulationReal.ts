...
export function stepSimulationReal(
  ...
  // =====================================================
  {
    const mode = ctx.simulationMode ?? "Normal"
  ...
  // =====================================================
  {
    ...
    const fatigueComponent = state.systemFatigue ?? 0

    const shockComponent = state.shockLevel ?? 0

    const momentumComponent = Math.abs(state.pressureMomentum ?? 0)
    ...
    if (!Number.isFinite(rawEnergy)) {
      ...
      const bounded = Math.max(0, Math.min(2, rawEnergy))

      // Saturación física suave
      state.systemEnergy = Math.tanh(bounded * 0.7)
    }
  }
  ...
  // =====================================================
  {
    if (state.engineTime >= (state.nextDemoEvent ?? 0)) {
      // ---------------------------------------------
      // CALM PERIOD
      // ---------------------------------------------
      if (state.engineTime < (state.calmUntil ?? 0)) {
        state.nextDemoEvent = state.engineTime + (12000 + engineRandom() * 8000)
        ...
        const pressure = state.unifiedPressure ?? 0

        const fatigue = state.systemFatigue ?? 0

        const scenario = ctx.simulationMode ?? "Normal"

        if (scenario === "Peak demand") {
          regionsToActivate =
            engineRandom() < 0.5 ? 1 : engineRandom() < 0.8 ? 2 : 3
        }

        if (scenario === "Expansion phase") {
          regionsToActivate = engineRandom() < 0.6 ? 1 : 2
        }

        state.pendingRegionalActivations = regionsToActivate
        state.nextSubEventTime = state.engineTime

        // ---------------------------------------------
        // NEXT EVENT TIMING
        // ---------------------------------------------
        let baseInterval = 10000 + engineRandom() * 10000

        // presión alta → más frecuencia
        baseInterval *= 1 - pressure * 0.3

        state.nextDemoEvent = state.engineTime + Math.max(4000, baseInterval)
      }
    }
    ...
    if (
      ...
      state.nextSubEventTime = state.engineTime + (800 + engineRandom() * 1400)
      ...
      if (regions.length > 0) {
        ...
        // ---------------------------------------------
        let index = Math.floor(engineRandom() * regions.length)

        const last = state.lastActiveRegion ?? null

        const streak = state.activityStreak ?? 0

        const clusterProbability = Math.min(0.6, 0.25 + streak * 0.08)

        if (last !== null && engineRandom() < clusterProbability) {
          index = last
        }
        ...
        if (region) {
          const capacity = Math.max(1, region.capacityKg)

          const utilization = region.availableKg / capacity
          ...
          downProbability = Math.max(0, Math.min(1, downProbability))
          ...
          // MAGNITUDE
          // ---------------------------------------------
          const baseMagnitude = 300 + engineRandom() * 900
          ...
          let nextVolume = region.availableKg + delta

          nextVolume = Math.max(0, Math.min(capacity, nextVolume))
          ...
          if (state.lastActiveRegion === index) {
            state.activityStreak = (state.activityStreak ?? 0) + 1
          } else {
  ...
  // =====================================================
  {
    const prev = state.pressureMomentum ?? 0

    const unifiedPressure = state.unifiedPressure ?? 0

    const stochastic = state.stochasticPressure ?? 0

    const systemEnergy = state.systemEnergy ?? 0

    const fatigue = state.systemFatigue ?? 0

    const drift = (state.regimeDriftSignal ?? 0.5) - 0.5

    const structuralInfluence = drift * 0.2

    const guardThrottle = state.guardThrottle ?? 1
    ...
    const pressureDeviation = unifiedPressure - equilibrium
    ...
    if (!Number.isFinite(next)) {
      ...
      state.pressureMomentum = Math.max(-0.2, Math.min(0.2, next))
    }
  }
  ...
  // =====================================================
  {
    ...
    const clusteringGain = PHYSICS.shock.clusteringGain

    const fragility =
      state.systemFatigue * 0.5 +
      state.systemEnergy * 0.5

    const shockActivity = state.shockActivity ?? 0
    ...
    if (engineRandom() < probability) {
      ...
      shockImpulse = amplitude * (1 + fragility * 0.5)
    }

    // Integración suave (absorción física)
    const smoothing = PHYSICS.shock.smoothing

    // Integración temporal coherente
    const shockDelta =
      (shockImpulse - (state.shockLevel ?? 0)) *
      smoothing
    ...
    const nextShockActivity =
      Math.max(0, shockActivity * 0.9 + shockImpulse * 2)

    if (!Number.isFinite(nextShockLevel)) {
      state.shockLevel = 0
    } else {
      state.shockLevel = Math.max(0, Math.min(1.5, nextShockLevel))
    }

    state.shockActivity = Math.max(0, Math.min(1, nextShockActivity))
  }
  ...
  // =====================================================
  {
    const currentFatigue = state.systemFatigue ?? 0
    ...
    const saturation =
      ...
      Math.min(
        1,
        fatigueLevel / PHYSICS.fatigue.maxFatigue
      )

    const pressure = state.unifiedPressure ?? 0

    const shock = state.shockLevel ?? 0
    ...
    let nextFatigue = currentFatigue + delta

    // Bound físico
    nextFatigue = Math.max(
      0,
      Math.min(PHYSICS.fatigue.maxFatigue, nextFatigue)
    )
  ...
  // =====================================================
  {
    const current = state.regimeDriftSignal ?? 0.5
  ...
  // =====================================================
  {
    const currentResilience = state.adaptiveResilience ?? 0.5
    ...
    let nextResilience =
      currentResilience +
      (learning - erosion) * dtNorm

    // Bound inferior estructural
    nextResilience =
      Math.max(PHYSICS.resilience.minimum, Math.min(1, nextResilience))

    if (!Number.isFinite(nextResilience)) {
      state.adaptiveResilience = PHYSICS.resilience.minimum
    } else {
      state.adaptiveResilience = nextResilience
    }
  }
  ...
  // =====================================================
  {
    const currentMemory = state.pathMemory ?? 0
    ...
    // Bound físico
    nextMemory = Math.max(0, Math.min(1, nextMemory))
  ...
  // =====================================================
  {
    const current = state.resilienceBudget ?? 0.8

    const resilience = state.adaptiveResilience ?? 0.5
  ...
  // =====================================================
  {
    const current = state.stressScar ?? 0

    const energy = state.systemEnergy ?? 0

    const collapse = state.collapseProximity ?? 0

    const slowing = state.criticalSlowing ?? 0

    // -----------------------------------------------------
    // MEMORY TIMESCALE
    // -----------------------------------------------------
    const memoryScale = 1 / PHYSICS.timescales.memory
    ...
    next = Math.max(0, Math.min(0.3, next))
  ...
  // =====================================================
  {
    const currentAttractor = state.attractorField ?? 0.5
    ...
    // Bound físico
    nextAttractor = Math.max(0, Math.min(1, nextAttractor))
  ...
  // =====================================================
  {
    ...
    state.regimeSwitchPressure =
      state.regimeSwitchPressure +
      (structuralPressure - state.regimeSwitchPressure) *
      0.05 *
      dtNorm
  ...
  // =====================================================
  {
    ...
    if (regions.length > 0) {
      ...
      for (const r of regions) {
        ...
        const utilization = 1 - (r.availableKg / capacity)
      ...
      const imbalancePenalty = imbalance * 0.3
  ...
  // =====================================================
  {
    ...
    state.collapseProbability =
      Math.max(0, Math.min(1, collapseRaw))
  }
  ...
  // =====================================================
  {
    const currentEnergy = state.systemEnergy ?? 0

    const previousEnergy = state.lastEnergySample ?? 0

    const energyDelta = Math.abs(currentEnergy - previousEnergy)

    // actualizar muestra histórica
    state.lastEnergySample = currentEnergy
