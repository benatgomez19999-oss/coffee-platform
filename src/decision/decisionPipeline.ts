...
const anticipatingShift =
  anticipationSignal > 0.4

if (anticipatingShift) {

  output.explanation.push(
    "Regime shift anticipated"
  )
...
if (patienceField < 0.3) {
  output.explanation.push("Low patience — environment fragile")
}
...
if (momentumMemory) {
  ...
  const delta =
    Math.abs(smoothedScore - prevScore)
